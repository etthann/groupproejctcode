...
float averageLandTemperatureEachMonth(char*** data, int rows, int columns, int targetMonth) {
    ...
    float averageTemp = (count > 0) ? totalTemp / count : 0.0;
    return averageTemp;
}
...
// Question 4
float hottestColdestMonth(char*** data, int row, int columns, int targetYear){

    float hottest, coldest, temp;
    ...
    for (int i = 0; i < row; i++){
        ...
        if (year >= 1760 && targetYear){
            char* temp = atoi(data[i][1];
