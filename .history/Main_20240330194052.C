...
void storeCSVData(char
*** data, int rows, int cols);
float averageLandTemperatureEachYear(char*** data, int rows, int cols, int targetYear);
void storeAnswerToFile(char*** data, int rows, int columns, const char* fileName, int startYear, int endYear);float averageLandTemperatureEachCentury(char*** data, int rows, int cols, int startYear, int endYear);
...
void storeAnswerToFile (char*** data, int rows, int columns, const char* fileName, int startYear, int endYear) {
    ...
    if (strcmp(fileName, "q1") == 0) {
        ...
        file = fopen(fullPath,"w");
        for (int i = 0; i < totalYear + 1; i++) {
            float averageTemp = averageLandTemperatureEachMonth(data, rows, columns, startYear + i);
            fprintf(file, "%d, %f\n", startYear + i, averageTemp);
        }  
    }
    fclose(file);
