...
void storeCSVData (char data[][MAX_COLS], int rows, int cols) {
    ...
    //Go through the entire file and store the values into a 2D array
    while (fgets(line,sizeof(line),file) && row < rows) {
        strncpy(data[row], line, MAX_COLS); // Copy the entire line
        data[row][MAX_COLS - 1] = '\0'; // Ensure the string is null-terminated
        row++;
    }
    fclose(file);
