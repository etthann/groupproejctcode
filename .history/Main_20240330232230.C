...
void hottestColdestMonth(char*** data, int row, int columns, float **listHottestColdest) {
    ...
    for (int i = 0; i < row; i++) {
        ...
        token = strtok(NULL, "-"); // Use NULL in subsequent calls to strtok
    ...
    printf("Hottest month: %d-%d with temperature %f\n", 
    (int)listHottestColdest[0][0], 
    (int)listHottestColdest[0][1], 
    listHottestColdest[0][2]);

    printf("Coldest month: %d-%d with temperature %f\n", 
        (int)listHottestColdest[1][0], 
        (int)listHottestColdest[1][1], 
        listHottestColdest[1][2]);
}
...
void storeAnswerToFile (char*** data, int rows, int columns, const char* fileName, int startYear, int endYear) {
    ...
    if (strcmp(fileName, "q1") == 0) {
        ...
        file = fopen(fullPath,"w");
        fprintf(file,"Hello");
        // fprintf(file, "Hottest Ever Recorded Month: %f, %f.\n Coldest Ever Recorded Month: %f, %f", listHottestColdest[0][1] ,listHottestColdest[0][0], listHottestColdest[1][1], listHottestColdest[1][0]);
