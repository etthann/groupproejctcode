...
float averageLandTemperature(char*** data, int rows, int cols, int targetYear);
void storeAnswerToFile(char*** data, int rows, int columns);
...
int main() {
    ...
    printf("The average land temperature is %f",averageLandTemperature(data,rows,columns,1760));
    storeAnswerToFile(data, rows, columns);
 ...
 * 
*/
float averageLandTemperature(char*** data, int row, int col, int targetYear) {
    int counter = 0;
    float totalTemp = 0.0;
...
void storeAnswerToFile (char*** data, int rows, int columns, char* fileName) {
    char folder [] = "answers/";
    char fullPath[100];
    char fileNameAnswerrs = (strcat(fol))

    file = fopen("q1.txt","w");
    if (file == NULL) {
        printf("Failed to open q1.txt");
        return;
    }
    for (int i = 0; i < 255; i++) {
        float averageTemp = averageLandTemperature(data, rows, columns, 1760 + i);
        fprintf(file, "%d, %f\n", 1760 + i, averageTemp);
    }
    fclose(file);
}