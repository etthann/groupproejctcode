...
float listHottestColdest[320][2];
...
//float hottestColdestYear(char*** data, int rows, int columns, int targetYear, floa)
...
void storeAnswerToFile (char*** data, int rows, int columns, const char* fileName, int startYear, int endYear) {
    ...
    if (strcmp(fileName, "q1") == 0) {
        ...
        for (int r = 0; r < 320; r++) {
            for (int c = 0; c < 2; r++) {
                fprintf(file, "Year: %f, Coldest Month: %f Hottest Month: %f", listHottestColdest)
            }
        }
    }
