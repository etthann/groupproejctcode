...
void storeAnswerToFile(char*** data, int rows, int columns, char* fileName, int startYear, int endYear, char answerArray[]);
...
int main() {
    ...
    storeAnswerToFile(data, rows, columns,"q1", 1760, 2015,NULL);


    //Question 2:
    storeAnswerToFile(data,rows,columns,"q2",1760,1799,NULL);
    storeAnswerToFile(data,rows,columns,"q2",1800,1899,NULL);
    storeAnswerToFile(data,rows,columns,"q2",1900,1999,NULL);
    storeAnswerToFile(data,rows,columns,"q2",2000,2015,NULL);

    // Question 3:
    float avgTempMonth [12];
    for (int i = 0; i < 12; i++) {
        avgTempMonth[i] = averageLandTemperatureEachMonth(data,rows,columns,i+1);
    }
    storeAnswerToFile(data,rows,columns,"q2",1760,1799,*avgTempMonth);
...
/**
 * Calculates the aaverage land temperature for each month
 * @param
 * @param
 * @param
 * @param
