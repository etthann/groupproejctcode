...
void hottestColdestMonth(char*** data, int row, int columns, int targetYear, float** list);
...
int main() {
    ...
    // Question 4
    listHottestColdest[0][1] = -10000;
    listHottestColdest[1][1] = 10000;
    listHottestColdest = hottestColdestMonth(data,rows,columns,listHottestColdest);
...
// Question 4
float hottestColdestMonth(char*** data, int row, int columns, float** listHottestColdest) {
    ...
    for (int i = 0; i < row; i++) {
        ...
        if (temp > listHottestColdest[0][1]){
            listHottestColdest[0][0] = year;
            listHottestColdest[0][1] = month;
        }
        
        if (temp < listHottestColdest[1][1]){
            coldest = temp;
...
void writeColumnsToFile(char*** data, int rows, int columns, const char* fileName) {
    FILE* file = fopen("averageMaxMinValues", "w");
    if (file == NULL) {
