...
void writeColumnsToFile(char*** data, int rows, int columns, const char fileName[]) {
    ...
    for (int r = 0; r < rows; r++) {
        ...
        if (year >= 1850 && year <= 2015) {
            ...
            yearData[index].sum3 += atof(data[r][5]);
...
void storeAnswerToFile (char*** data, int rows, int columns, const char* fileName, int startYear, int endYear) {
    ...
    //If fileName == q2
        ...
        // Open the file and append to the file 
        // Now there is some error with this as we can only run the code once if we run more than once then it would write the same results more than once
        file = fopen(fullPath,"a");
        // Calculate the average land temperature for each century
        float averageTemp = averageLandTemperatureEachCentury(data,rows,columns,startYear,endYear);
        // Print the value and the century it's in into the file
        fprintf(file,"%dth Century, %f\n",(startYear/100) + 1,averageTemp);



    //Store the answers from question 3 into the q3 file
    ...
    //If fileName == q3
        ...
        //Loop 12 times
       for (int i = 1; i < 13; i++) {
            // Calculate the average Land temperature for each month
            float averageTemp = averageLandTemperatureEachMonth(data, rows, columns, i);
            //Store the month and it's average land temperature to the file
            fprintf(file, "%s, %f\n", month[i-1], averageTemp);
        }



    //Store the answers from question 4 into the q4 file

    //If fileName == q4
    }  if (strcmp(fileName,"q4") == 0) {

        //Dynamically allocate data into the 2D float array listHotttestColdest
        ...
        //Print the hottest and coldest month with it's year and temperature
        ...
        //Free up any allocated memory using the free function
        for (int i = 0; i < 2; i++) {
    ...
    //Store the answers from question 11 into the q11 file

        //If fileName == q11
     } if (strcmp(fileName, "q11") == 0) {
        //Open up the file and write to it
        file = fopen(fullPath,"w");
        //Loop through the function, totalYear + 1 times and calculate the averageTemp and averageocean temp and write those values into the file
        for (int i = 0; i < totalYear + 1; i++) {
            float averageTemp = averageLandTemperatureEachYear(data, rows, columns, startYear + i);
