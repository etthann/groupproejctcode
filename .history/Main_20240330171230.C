...
int main() {
    ...
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < columns; c++) {
            printf("%s ", data[r][1]); // Print each string in the array
        }
    ...
    // printf("The average land temperature is %f",averageLandTemperature(data,rows,columns,1000));
 ...
 * 
*/
float averageLandTemperatureForYear(char*** data, int row, int col, int targetYear) {
    int counter = 0;
    float totalTemp = 0.0;
    ...
    for (int r = 0; r < row; r++) {
        ...
        if (year == targetYear) {
            ...
            for (int c = 1; c < col; c++) {
                ...
                if (temp != 0.0) { // assuming that 0.0 means no data available
                    monthlyTemp += temp;
                    monthCount++;
                }
            }
    ...
    // () condition to check, ? for if the condition is true, : for condition false
