...
int main() {
    ...
    //Don't touch anything else down here except for printing the 3D array
...
float averageLandTemperature(char*** data, int row, int col) {
    float avgLandTemp = 0;
    int totalYear = 2015-1760;
    for (int r = 0; r < row; r++) {
        for (int c = 0; c < col; c++) {
            avgLandTemp = data[r][c][1];
        }
    }
    printf("The avgerage land temperature is %f", avgLandTemp);
    return 0;
}