...
void storeAnswerToFile (char*** data, int rows, int columns, char* fileName, int startYear, int endYear) {
    ...
    if (strcmp(fileName, "q1") == 0) {
        ...
        for (int centuryStart = startYear; centuryStart <= endYear; centuryStart += 100) {
            int centuryEnd = centuryStart + 99;
            if (centuryEnd > endYear) {
                centuryEnd = endYear;
            }
            float averageTemp = averageLandTemperatureEachCentury(data, rows, columns, centuryStart, centuryEnd);
            fprintf(file, "%dth Century, %f\n", centuryStart / 100, averageTemp);
        }
    }
    fclose(file);
}
