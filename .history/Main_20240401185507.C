...
float averagelandAndOceanTemperatures(char*** data, int row, int col, int targetYear) {
    ...
    //If counter is greater than 0 then calculate the average temp, if not then averageOceanTemp is 0
    landAndOceanAverageTemperature  = (counter > 0) ? totalTemp / counter : 0;
    
    //Returns the averageOceanTemp variable which holds the average temp of that year
