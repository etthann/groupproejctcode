...
float averageLandTemperatureEachCentury(char*** data, int rows, int columns, int startYear, int endYear) {
    ...
    averageLandTemp = (counter > 0) ? totalTemp / counter : 0; // basically a if else statement that checks if counter if greater than 1 and will find the average else return 
    return averageLandTemp;
}
