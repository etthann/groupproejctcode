...
float averageLandTemperatureEachCentury(char*** data, int rows, int columns, int startYear, int endYear) {
    ...
    averageLandTemp = (counter > 0) ? totalTemp / counter : 0; // basically a if else statement that checks if counter if greater than 1 and will find the average else return 0
    return averageLandTemp; // returun final average value
}
...
void hottestColdestMonth(char*** data, int row, int columns, float **listHottestColdest) {
    ...
    for (int r = 0; r < row; r++) {
        ...
        if (temp > listHottestColdest[0][2]){
            ...
            listHottestColdest[0][1] = month;
            listHottestColdest[0][2] = temp; // Update hottest temperature
        }
        
        if (temp < listHottestColdest[1][2]){
            listHottestColdest[1][0] = year;
            listHottestColdest[1][1] = month;
            listHottestColdest[1][2] = temp; // Update coldest temperature
        }
    }
    printf("Hottest Month: Year = %.0f, Month = %d, Temperature = %f\n", listHottestColdest[0][0], (int) listHottestColdest[0][1], listHottestColdest[0][2]);
    printf("Coldest Month: Year = %.0f, Month = %d, Temperature = %f\n", listHottestColdest[1][0], (int) listHottestColdest[1][1], listHottestColdest[1][2]);
}
 ...
 */
void writeColumnsToFile(char*** data, int rows, int columns, const char fileName[]) {
    YearData yearData[300] = {0};  // Assuming the data spans no more than 300 years

    // Accumulate the values and counts for each year
    for (int r = 0; r < rows; r++) {
        char dateCopy[100];
        strcpy(dateCopy, data[r][0]);
        char *token = strtok(dateCopy, "-");
    ...
    // Compute the averages and write them to the file
